...
class SassFunction(object):
    ...
    def __init__(self, name, arguments, callable_):
        if not isinstance(name, string_types):
            ...
            raise TypeError("arguments must be a sequence, not " + repr(arguments),)
        elif not callable(callable_):
            raise TypeError(repr(callable_) + " is not callable")
        self.name = name
        self.arguments = tuple(
            arg if arg.startswith("$") else "$" + arg for arg in arguments
        )
...
def compile_dirname(
    ...
    for dirpath, _, filenames in os.walk(search_path, onerror=_raise):
        filenames = [
            ...
            if filename.endswith((".scss", ".sass")) and not filename.startswith("_")
        ]
        for filename in filenames:
    ...
    if kwargs:
        raise TypeError(
            "{}() got unexpected keyword argument(s) {}".format(
                func.__name__, ", ".join("'{}'".format(arg) for arg in sorted(kwargs)),
            ),
        )
...
class SassList(
    collections.namedtuple("SassList", ("items", "separator", "bracketed"),),
):
