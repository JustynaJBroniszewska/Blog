...
def validate_manifests(dist, attr, value):
    ...
    try:
        ...
        raise distutils.errors.DistutilsSetupError(
            ...
            "{'package.name': ('sass/path', 'css/path'}), not " + repr(value),
        )
...
class build_sass(Command):
    ...
    def run(self):
        ...
        for package_name, manifest in manifests.items():
            ...
            css_files = manifest.build(package_dir, output_style=self.output_style,)
            map(distutils.log.info, css_files)
            package_data.setdefault(package_name, []).extend(css_files)
            data_files.append(
                (package_dir, [os.path.join(package_dir, f) for f in css_files],),
            )
