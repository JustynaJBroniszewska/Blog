...
def build_directory(
    ...
    for name in os.listdir(sass_path):
        ...
        if SUFFIX_PATTERN.search(name) and os.path.isfile(sass_fullname):
            ...
            with io.open(css_fullname, "w", encoding="utf-8", newline="",) as css_file:
                css_file.write(css)
            result[os.path.relpath(sass_fullname, _root_sass)] = os.path.relpath(
                css_fullname, _root_css
            )
...
class Manifest(object):
    ...
    def normalize_manifests(cls, manifests):
        if manifests is None:
            ...
            raise TypeError(
                "manifests must be a mapping object, not " + repr(manifests),
            )
    ...
    def __init__(
        self, sass_path, css_path=None, wsgi_path=None, strip_extension=None,
    ):
        if not isinstance(sass_path, string_types):
            raise TypeError("sass_path must be a string, not " + repr(sass_path),)
        if css_path is None:
            css_path = sass_path
        elif not isinstance(css_path, string_types):
            raise TypeError("css_path must be a string, not " + repr(css_path),)
        if wsgi_path is None:
            wsgi_path = css_path
        elif not isinstance(wsgi_path, string_types):
            raise TypeError("wsgi_path must be a string, not " + repr(wsgi_path),)
        if strip_extension is None:
            ...
            raise TypeError(
                "strip_extension must be bool not {!r}".format(strip_extension,),
            )
    ...
    def unresolve_filename(self, package_dir, filename):
        ...
        if self.strip_extension:
            for ext in (".scss", ".sass"):
                test_path = os.path.join(package_dir, self.sass_path, filename + ext,)
    ...
    def build(self, package_dir, output_style="nested"):
        ...
        return frozenset(
            os.path.join(self.css_path, filename) for filename in css_files
        )
    ...
    def build_one(self, package_dir, filename, source_map=False):
        ...
        """
        sass_filename, css_filename = self.resolve_filename(package_dir, filename,)
        ...
        if source_map:
            ...
            with io.open(source_map_path, "w", encoding="utf-8", newline="",) as f:
                f.write(source_map)
        return css_filename
