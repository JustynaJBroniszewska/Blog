...
class SassMiddleware(object):
    ...
    def __init__(
        self, app, manifests, package_dir={}, error_status="200 OK",
    ):
        if not callable(app):
            raise TypeError(
                "app must be a WSGI-compliant callable object, " "not " + repr(app),
            )
        self.app = app
        self.manifests = Manifest.normalize_manifests(manifests)
        if not isinstance(package_dir, collections_abc.Mapping):
            raise TypeError(
                "package_dir must be a mapping object, not " + repr(package_dir),
            )
            ...
            for prefix, package_dir, manifest in self.paths:
                ...
                try:
                    result = manifest.build_one(
                        package_dir, sass_filename, source_map=True,
                    )
