...
from django.template.loader import (
    get_template,
)  # in order to preload template locations
...
class Command(BaseCommand):
    ...
    def __init__(self):
        ...
        self.sass_output_style = getattr(
            settings, "SASS_OUTPUT_STYLE", "nested" if settings.DEBUG else "compressed"
        )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=False,
            help=_(
                "Store resulting .css in settings.SASS_PROCESSOR_ROOT folder. "
    ...
    def get_parser(self, engine):
        if engine == "jinja2":
            ...
            raise CommandError(
                "Invalid templating engine '{engine}' specified.".format(engine=engine)
            )
        return parser
    ...
    def handle(self, *args, **options):
        ...
        for engine in engines:
            ...
            try:
                self.sass_precision = int(
                    options["sass_precision"] or settings.SASS_PRECISION
                )
            ...
            for py_source in self.find_sources():
                ...
                try:
                    ...
                    self.stderr.write(
                        "Syntax error encountered processing %s" % py_source
                    )
    ...
    def find_templates(self):
        ...
        for loader in self.get_loaders():
            try:
                ...
                paths.update(
                    [t.name if isinstance(t, Origin) else t for t in template_sources]
                )
        ...
        for path in paths:
            for root, _, files in os.walk(str(path)):
                templates.update(
                    ...
                    and any(name.endswith(ext) for ext in self.template_exts)
                )
        if not templates:
            raise CommandError(
    ...
    def parse_template(self, template_name):
        try:
            ...
            if self.verbosity > 0:
                self.stderr.write(
                    "\nUnicodeDecodeError while trying to read template {}".format(
                        template_name
                    )
                )
        try:
    ...
    def delete_file(self, sass_filename, sass_fileurl):
        ...
        if self.use_static_root:
            destpath = os.path.join(
                self.static_root, os.path.splitext(sass_fileurl)[0] + ".css"
            )
        else:
