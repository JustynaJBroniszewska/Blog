...
def main(argv=sys.argv, stdout=sys.stdout, stderr=sys.stderr):
    ...
    parser.add_option(
        ...
        metavar="STYLE",
        type="choice",
        choices=list(sass.OUTPUT_STYLES),
        default="nested",
        help=(
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Emit source map.  Requires the second argument " "(output css filename).",
    )
    parser.add_option(
        "--sourcemap-file",
    ...
    parser.add_option(
        ...
        dest="include_paths",
        action="append",
        help='Path to find "@import"ed (S)CSS source files. ' "Can be multiply used.",
    )
    parser.add_option(
        "-p",
        "--precision",
    ...
    error = functools.partial(print, parser.get_prog_name() + ": error:", file=stderr,)
    if not args:
    ...
    try:
        ...
        if source_map_filename:
            with io.open(source_map_filename, "w", encoding="utf-8", newline="",) as f:
                f.write(source_map)
    return 0
