...
class SassTestCase(BaseTestCase):
    ...
    def test_and_join(self):
        ...
        self.assertEqual(
            "Korea, and Japan", sass.and_join(["Korea", "Japan"]),
        )
...
class CompileTestCase(BaseTestCase):
    ...
    def test_compile_exclusive_arguments(self):
        ...
        self.assertRaises(
            TypeError, sass.compile, string="a { color: blue; }", dirname="test/",
        )
        self.assertRaises(
            TypeError, sass.compile, filename="test/a.scss", dirname="test/",
        )
    ...
    def test_compile_invalid_output_style(self):
        ...
        self.assertRaises(
            TypeError, sass.compile, string="a { color: blue; }", output_style=123j,
        )
        self.assertRaises(
            ValueError,
            sass.compile,
    ...
    def test_compile_invalid_source_comments(self):
        ...
        self.assertRaises(
            TypeError, sass.compile, string="a { color: blue; }", source_comments=123j,
        )
        self.assertRaises(
            TypeError,
            sass.compile,
...
""",
        ...
        self.assertRaises(
            sass.CompileError, sass.compile, string="a { b { color: blue; }",
        )
        # sass.CompileError should be a subtype of ValueError
        self.assertRaises(
            ValueError, sass.compile, string="a { b { color: blue; }",
        )
    ...
    def test_importers_other_iterables(self):
        ...
        assert ret == ("a{color:red}b{color:orange}c{color:yellow}d{color:green}\n")
    ...
    def test_compile_string_deprecated_source_comments_line_numbers(self):
        ...
        with pytest.warns(FutureWarning):
            actual = sass.compile(string=source, source_comments="line_numbers",)
        assert expected == actual
    ...
    def test_compile_filename(self):
        ...
        self.assertRaises(
            IOError, sass.compile, filename="test/not-exist.sass",
        )
    ...
    def test_compile_source_map(self):
        ...
        actual, source_map = sass.compile(
            filename=filename, source_map_filename="a.scss.css.map",
        )
...
class BuilderTestCase(BaseTestCase):
    ...
    def test_builder_build_directory(self):
        ...
        with io.open(os.path.join(css_path, "a.scss.css"), encoding="UTF-8",) as f:
            css = f.read()
        assert A_EXPECTED_CSS == css
        assert "b.scss.css" == result_files["b.scss"]
        with io.open(os.path.join(css_path, "b.scss.css"), encoding="UTF-8",) as f:
            css = f.read()
        assert B_EXPECTED_CSS == css
        assert "c.scss.css" == result_files["c.scss"]
        with io.open(os.path.join(css_path, "c.scss.css"), encoding="UTF-8",) as f:
            css = f.read()
        assert C_EXPECTED_CSS == css
        assert "d.scss.css" == result_files["d.scss"]
        with io.open(os.path.join(css_path, "d.scss.css"), encoding="UTF-8",) as f:
            css = f.read()
        assert D_EXPECTED_CSS == css
        assert "e.scss.css" == result_files["e.scss"]
        with io.open(os.path.join(css_path, "e.scss.css"), encoding="UTF-8",) as f:
            css = f.read()
        ...
        with io.open(os.path.join(css_path, "g.scss.css"), encoding="UTF-8",) as f:
            css = f.read()
        ...
        with io.open(os.path.join(css_path, "h.sass.css"), encoding="UTF-8",) as f:
            css = f.read()
        assert H_EXPECTED_CSS == css
        with io.open(
            os.path.join(css_path, "subdir", "recur.scss.css"), encoding="UTF-8",
        ) as f:
            css = f.read()
    ...
    def test_output_style(self):
        ...
        result_files = build_directory(
            self.sass_path, css_path, output_style="compressed",
        )
        assert len(result_files) == 8
        assert "a.scss.css" == result_files["a.scss"]
        with io.open(os.path.join(css_path, "a.scss.css"), encoding="UTF-8",) as f:
...
class ManifestTestCase(BaseTestCase):
    ...
    def test_build_one(self):
        with tempdir() as d:
            ...
            with io.open(os.path.join(d, "css", "b.scss.css"), encoding="UTF-8",) as f:
            ...
            self.assert_source_map_file(
                {
                    ...
                    "names": [],
                    "mappings": (
                        "AAAA,AACE,CADD,CACC,CAAC,CAAC;EACA,SAAS,EAAE,IAAI," "GAChB"
                    ),
                },
                os.path.join(d, "css", "b.scss.css.map"),
            )
            m.build_one(d, "d.scss", source_map=True)
            with io.open(os.path.join(d, "css", "d.scss.css"), encoding="UTF-8",) as f:
...
class WsgiTestCase(BaseTestCase):
    ...
    def test_wsgi_sass_middleware(self):
        with tempdir() as css_dir:
            ...
            with pytest.warns(FutureWarning):
                app = SassMiddleware(
                    self.sample_wsgi_app, {__name__: (src_dir, css_dir, "/static"),},
                )
            ...
            assert r.status_code == 200
            self.assertEqual(
                b(_map_in_output_dir(A_EXPECTED_CSS_WITH_MAP)), r.data,
            )
...
class DistutilsTestCase(BaseTestCase):
    ...
    def css_path(self, *args):
        return os.path.join(
            os.path.dirname(__file__), "testpkg", "testpkg", "static", "css", *args
        )
    ...
    def test_build_sass(self):
        ...
        assert rv == 0
        self.assertEqual(
            ["a.scss.css"], list(map(os.path.basename, self.list_built_css())),
        )
...
class SasscTestCase(BaseTestCase):
    ...
    def test_three_args(self):
        exit_code = pysassc.main(
            ["pysassc", "a.scss", "b.scss", "c.scss"], self.out, self.err,
        )
        assert exit_code == 2
        err = self.err.getvalue()
        assert err.strip().endswith(
    ...
    def test_pysassc_stdout(self):
        exit_code = pysassc.main(["pysassc", "test/a.scss"], self.out, self.err,)
        assert exit_code == 0
    ...
    def test_pysassc_output(self):
        ...
        try:
            os.close(fd)
            exit_code = pysassc.main(
                ["pysassc", "test/a.scss", tmp], self.out, self.err,
            )
            assert exit_code == 0
            assert self.err.getvalue() == ""
            assert self.out.getvalue() == ""
    ...
    def test_pysassc_output_unicode(self):
        ...
        try:
            os.close(fd)
            exit_code = pysassc.main(
                ["pysassc", "test/d.scss", tmp], self.out, self.err,
            )
            assert exit_code == 0
            assert self.err.getvalue() == ""
            assert self.out.getvalue() == ""
    ...
    def test_pysassc_source_map_without_css_filename(self):
        exit_code = pysassc.main(["pysassc", "-m", "a.scss"], self.out, self.err,)
        assert exit_code == 2
        err = self.err.getvalue()
        assert err.strip().endswith(
    ...
    def test_pysassc_warning_import_extensions(self):
        with pytest.warns(FutureWarning):
            pysassc.main(["pysassc", os.devnull, "--import-extensions", ".css"],)
...
class CompileDirectoriesTest(unittest.TestCase):
    ...
    def test_successful(self):
        with tempdir() as tmpdir:
            ...
            write_file(
                os.path.join(input_dir, "f1.scss"), "a { b { width: 100%; } }",
            )
            write_file(
                os.path.join(input_dir, "foo/f2.scss"), "foo { width: 100%; }",
            )
...
class SassFunctionTest(unittest.TestCase):
    ...
    def test_sigature(self):
        sf = sass.SassFunction(  # pragma: no branch (doesn't run lambda)
            "func-name", ("$a", "$bc", "$d"), lambda a, bc, d: None,
        )
...
@pytest.mark.parametrize(  # pragma: no branch (never runs lambdas)
    "func", (lambda bar="womp": None, lambda *args: None, lambda **kwargs: None),
)
...
class SassTypesTest(unittest.TestCase):
    def test_number_no_conversion(self):
        num = sass.SassNumber(123.0, u"px")
    ...
    def test_color_no_conversion(self):
        color = sass.SassColor(1.0, 2.0, 3.0, 0.5)
...
def returns_color():
    return sass.SassColor(1, 2, 3, 0.5)
...
def returns_bracketed_list():
    return sass.SassList(("hello", "ohai"), sass.SASS_SEPARATOR_SPACE, bracketed=True,)
...
def compile_with_func(s):
    result = sass.compile(
        string=s, custom_functions=custom_functions, output_style="compressed",
    )
    map_result = sass.compile(
        string=s, custom_functions=custom_function_map, output_style="compressed",
    )
    assert result == map_result
    set_result = sass.compile(
        string=s, custom_functions=custom_function_set, output_style="compressed",
    )
...
class CustomFunctionsTest(unittest.TestCase):
    ...
    def test_true(self):
        self.assertEqual(
            compile_with_func("a { content: returns_true(); }"), "a{content:true}\n",
        )

    def test_false(self):
        self.assertEqual(
            compile_with_func("a { content: returns_false(); }"), "a{content:false}\n",
        )
    ...
    def test_number(self):
        self.assertEqual(
            compile_with_func("a { width: returns_number(); }"), "a{width:5px}\n",
        )
    ...
    def test_py_dict(self):
        self.assertEqual(
            compile_with_func("a { content: map-get(returns_py_dict(), foo); }",),
            "a{content:bar}\n",
        )

    def test_map(self):
        self.assertEqual(
            compile_with_func("a { content: map-get(returns_map(), foo); }",),
            "a{content:bar}\n",
        )

    def test_identity_none(self):
        self.assertEqual(
            compile_with_func("a {color: #fff; content: identity(returns_none());}",),
            "a{color:#fff}\n",
        )
    ...
    def test_identity_comma_list(self):
        self.assertEqual(
            compile_with_func("a { font-family: identity(returns_comma_list()); }",),
            "a{font-family:Arial,sans-serif}\n",
        )

    def test_identity_space_list(self):
        self.assertEqual(
            compile_with_func("a { border-right: identity(returns_space_list()); }",),
            "a{border-right:medium none}\n",
        )

    def test_identity_bracketed_list(self):
        self.assertEqual(
            compile_with_func("a { content: identity(returns_bracketed_list()); }",),
            "a{content:[hello ohai]}\n",
        )
    ...
    def test_identity_map(self):
        self.assertEqual(
            compile_with_func("a { content: map-get(identity(returns_map()), foo); }",),
            "a{content:bar}\n",
        )
...
def test_pysassc_sourcemap(tmpdir):
    ...
    exit_code = pysassc.main(["pysassc", "-m", src_file.strpath, out_file.strpath,])
    assert exit_code == 0
