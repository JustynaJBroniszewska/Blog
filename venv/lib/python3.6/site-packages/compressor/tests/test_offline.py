...
class OfflineTestCaseMixin(object):
    ...
    def setUp(self):
        ...
        if "jinja2" in self.engines:
            ...
            with io.open(
                self.template_path_jinja2, encoding=settings.FILE_CHARSET
            ) as file_:
    ...
    def _render_script(self, hash):
        return (
            ...
            "</script>".format(settings.COMPRESS_URL_PLACEHOLDER, hash)
        )

    def _render_link(self, hash):
        return (
            '<link rel="stylesheet" href="{}CACHE/css/{}.css" '
            'type="text/css" />'.format(settings.COMPRESS_URL_PLACEHOLDER, hash)
        )
...
class OfflineCompressBlockSuperTestCaseWithExtraContent(OfflineTestCaseMixin, TestCase):
    ...
    def _test_offline(self, engine):
        ...
        self.assertEqual(
            [self._render_script("9717f9c7e9ff"), self._render_script("68c645740177")],
            result,
        )
        rendered_template = self._render_template(engine)
        self.assertEqual(rendered_template, self._render_result(result, ""))
...
class OfflineCompressConditionTestCase(OfflineTestCaseMixin, TestCase):
    ...
    additional_test_settings = {"COMPRESS_OFFLINE_CONTEXT": {"condition": "red",}}
...
class OfflineCompressTestCaseWithContext(OfflineTestCaseMixin, TestCase):
    ...
    additional_test_settings = {"COMPRESS_OFFLINE_CONTEXT": {"content": "OK!",}}
...
class OfflineCompressTestCaseWithContextVariableInheritance(
    ...
    additional_test_settings = {
        "COMPRESS_OFFLINE_CONTEXT": {"parent_template": "base.html",}
    }
...
class OfflineCompressInlineNonAsciiTestCase(OfflineTestCaseMixin, TestCase):
    ...
    additional_test_settings = {
        "COMPRESS_OFFLINE_CONTEXT": {"test_non_ascii_value": "\u2014",}
    }
