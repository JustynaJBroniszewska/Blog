...
class PrecompilerAndAbsoluteFilterTestCase(SimpleTestCase):
    ...
    def test_precompiler_enables_absolute(self):
        ...
        self.helper(
            ...
            expected_output=self.html_orig,
        )
        self.helper(
            enabled=False,
            use_precompiler=False,
...
<script type="text/javascript">obj.value = "value";</script>"""
    ...
    def test_custom_output_dir2(self):
        output = (
            '<script type="text/javascript" src="/static/js/74e158ccb432.js"></script>'
        )
        self.assertEqual(output, JsCompressor(self.js).output())
...
<style type="text/foobar" media="screen">h1 { border:5px solid green;}</style>"""
        ...
        self.assertEqual(
            ["screen", "screen", "screen"], [l.get("media", None) for l in output]
        )
...
class CompressorInDebugModeTestCase(SimpleTestCase):
    ...
    def test_filename_in_debug_mode(self):
        ...
        with open(
            os.path.join(settings.COMPRESS_ROOT, "CACHE", "css", "%s.css" % hashed), "r"
        ) as f:
            result = f.read()
